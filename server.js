...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (requestUrl.pathname === "/api/analyze-blood-test" && req.method === "POST") {
...
async function handleAnalyzeBloodTest(req, res) {
  if (!OPENAI_API_KEY) {
    sendJson(res, 500, { error: "OPENAI_API_KEY is missing. Add it to .env before using blood test analysis." });
    return;
  }
  ...
  try {
    const insight = await requestBloodTestAnalysis({ report, nutrition, workoutFrequency });
...
async function requestBloodTestAnalysis({ report, nutrition, workoutFrequency }) {
  const payload = buildBloodAnalysisPayload({ report, nutrition, workoutFrequency });
  const response = await fetch("https://api.openai.com/v1/responses", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error("OpenAI blood analysis request failed", response.status, errorText);
  ...
  const outputText = extractOutputText(result);
  if (!outputText) {
    console.error("OpenAI blood analysis missing output text", JSON.stringify(result));
  ...
  try {
    ...
    console.error("Failed to parse blood analysis JSON", outputText);
...
function buildBloodAnalysisPayload({ report, nutrition, workoutFrequency }) {
        ...
        content: [
          {
            ...
            text: [
              ...
              "Never be alarmist. Be supportive and constructive.",
              "If the report has no abnormal markers, be positive and brief.",
              "Do not diagnose conditions. Focus on calm interpretation and practical habits.",
